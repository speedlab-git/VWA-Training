...
class MWSPOArguments:
    r"""Arguments pertaining to the m-WSPO training stage.

    See `agent.md` (Eqs. 6, 8, 9, 10, 11) for the full objective. The paper
    overloads `beta`; we split the DPO temperature inside `sigma(.)` (Eq. 10)
    from the weight on `L_DPO^m` in unified Eq. 11:

    - `dpo_temperature` is the temperature inside `sigma(.)`  (Eq. 10).
    - `beta_dpo`        is the weight on  `L_DPO^m`           (Eq. 11).
    """

    mwspo_gamma: float = field(
        default=0.1,
        metadata={"help": "WSPO regularization gamma in Eqs. 6, 8, 9 (called 'beta' in the original WSPO repo)."},
    )
    mwspo_alpha: float = field(
        default=0.5,
        metadata={"help": "Weight on L_img_WSPO in unified Eq. 11."},
    )
    mwspo_beta_dpo: float = field(
        default=1.0,
        metadata={"help": "Weight on L_DPO^m in unified Eq. 11."},
    )
    mwspo_dpo_temperature: float = field(
        default=0.1,
        metadata={"help": "Temperature inside sigma(.) for L_DPO^m (Eq. 10)."},
    )
    mwspo_weak_ref_model: str | None = field(
        default=None,
        metadata={"help": "Path or HF id of the (frozen) weak reference MLLM pi_ref^w."},
    )
    mwspo_weak_aligned_model: str | None = field(
        default=None,
        metadata={"help": "Path or HF id of the (frozen) DPO-aligned weak MLLM pi_r^w."},
    )
    mwspo_weak_ref_adapters: str | None = field(
        default=None,
        metadata={"help": "Optional adapter path(s) for the weak reference model."},
    )
    mwspo_weak_aligned_adapters: str | None = field(
        default=None,
        metadata={"help": "Optional adapter path(s) for the DPO-aligned weak model."},
    )
    mwspo_neg_image_strategy: Literal["in_batch", "pre_mined"] = field(
        default="in_batch",
        metadata={
            "help": (
                "How to obtain the mismatched image m_l: 'in_batch' rotates images by 1 within the batch, "
                "'pre_mined' reads an `images_neg` column from the dataset (must be added in dataset_info.json)."
            )
        },
    )
    mwspo_disable_wspo: bool = field(
        default=False,
        metadata={"help": "Ablation: drop L_WSPO (Eq. 6)."},
    )
    mwspo_disable_img_wspo: bool = field(
        default=False,
        metadata={"help": "Ablation: drop L_img_WSPO winning + losing terms (Eqs. 8, 9). Equivalent to alpha=0."},
    )
    mwspo_disable_dpo: bool = field(
        default=False,
        metadata={"help": "Ablation: drop L_DPO^m (Eq. 10). Equivalent to beta_dpo=0."},
    )
    mwspo_mask_image_in_dpo: bool = field(
        default=False,
        metadata={
            "help": (
                "Sanity ablation: replace L_DPO^m with text-only DPO by zeroing pixel_values for the DPO term. "
                "Validates that visual conditioning matters."
            )
        },
    )
    mwspo_cache_weak_logprobs: bool = field(
        default=False,
        metadata={"help": "If true, read pre-computed weak logp values from `mwspo_weak_logprob_cache`."},
    )
    mwspo_weak_logprob_cache: str | None = field(
        default=None,
        metadata={"help": "Path to a parquet file with cached weak (aligned/ref) logps keyed by sample id."},
    )
...
class FinetuningArguments(
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "dpo", "kto", "m_wspo"] = field(
        default="sft",
    ...
    def __post_init__(self):
        ...
        self.use_ref_model = (self.stage == "dpo" and self.pref_loss not in ["orpo", "simpo"]) or (
            self.stage == "m_wspo"
        )

        if self.stage == "m_wspo":
            if self.mwspo_cache_weak_logprobs:
                if self.mwspo_weak_logprob_cache is None:
                    raise ValueError(
                        "`mwspo_weak_logprob_cache` must be set when `mwspo_cache_weak_logprobs=True`."
                    )
                # weak models are optional when a full cache is provided;
                # otherwise the trainer will fall back to live weak forwards.
            else:
                if self.mwspo_weak_ref_model is None or self.mwspo_weak_aligned_model is None:
                    raise ValueError(
                        "Both `mwspo_weak_ref_model` and `mwspo_weak_aligned_model` are required for "
                        "stage 'm_wspo' (or set `mwspo_cache_weak_logprobs=True` with "
                        "`mwspo_weak_logprob_cache` pointing at a full parquet)."
                    )
