...
def _get_preprocessed_dataset(
    ...
    # Inject a stable per-row sample id so downstream consumers (in particular
    # the m-WSPO offline weak-logprob cache) can key into the cache by example.
    # IterableDatasets don't support `add_column`, so we skip there; cache
    # support requires non-streaming mode.
    if isinstance(dataset, Dataset) and "_sample_id" not in dataset.column_names:
        dataset = dataset.add_column("_sample_id", list(range(len(dataset))))
