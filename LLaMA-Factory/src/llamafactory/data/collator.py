...
# Keys produced by the multimodal pipeline that depend on the image input;
# these get duplicated under "neg_*" by `MWSPOPairwiseDataCollator` for the
# mismatched-image (m_l) forward pass.
_MM_IMAGE_DERIVED_KEYS = (
    "input_ids",
    "attention_mask",
    "labels",
    "position_ids",
...
@dataclass
class MWSPOPairwiseDataCollator(PairwiseDataCollatorWithPadding):
    r"""Pairwise multimodal collator that additionally emits a parallel batch
    where each sample's image `m_w` is replaced by a mismatched image `m_l`.

    The mismatched-image batch is collated independently (so input_ids /
    image_grid_thw / pixel_values are all internally consistent for the m_l
    pass) and exposed under `neg_*` keys. Both the chosen-text and
    rejected-text branches are produced for the m_l pass; downstream losses
    (`L_img_WSPO^winning` Eq. 8) only use the chosen half.

    Two strategies for choosing m_l (set via `MWSPOArguments.mwspo_neg_image_strategy`):

    1. ``in_batch`` (default) -- shift the per-sample image list by 1 inside
       the batch (m_l_i := images_{(i+1) mod B}). Cheap, no preprocessing.
    2. ``pre_mined``  -- use the dataset-provided `images_neg` column. Falls
       back to in-batch shifting if the column is absent for a given sample.

    Stable per-sample IDs assigned in `_get_preprocessed_dataset` are
    surfaced as `sample_id` on the batch (length B, NOT replicated across
    chosen+rejected). Used for keying into the offline weak-logprob cache.
    """

    neg_image_strategy: Literal["in_batch", "pre_mined"] = "in_batch"

    @staticmethod
    def _rotate(items: list[Any]) -> list[Any]:
        if len(items) <= 1:
            return list(items)
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        # Pop m-WSPO-specific fields before delegating to the multimodal
        # collator (which doesn't know about them).
        per_sample_pos_images: list[list[Any]] = []
        per_sample_neg_images: list[list[Any] | None] = []
        sample_ids: list[int] = []
        has_sample_id = all("sample_id" in f for f in features)
        for feature in features:
            per_sample_pos_images.append(list(feature.get("images") or []))
            raw_neg = feature.pop("images_neg", None)
            per_sample_neg_images.append(list(raw_neg) if raw_neg else None)
            if has_sample_id:
                sample_ids.append(int(feature.pop("sample_id")))

        # Determine per-sample m_l image lists.
        if self.neg_image_strategy == "pre_mined" and any(neg is not None for neg in per_sample_neg_images):
            neg_images_per_sample = [
                neg if neg is not None else pos for neg, pos in zip(per_sample_neg_images, per_sample_pos_images)
            ]
        else:
            neg_images_per_sample = self._rotate(per_sample_pos_images)

        # 1) Standard m_w (chosen + rejected) pass.
        ...
        pos_batch = MultiModalDataCollatorForSeq2Seq.__call__(self, pos_features)

        # 2) Mismatched m_l pass -- same chosen/rejected text, swapped images.
                ...
                neg_features.append(
                    {
                        ...
                        "attention_mask": feature[f"{key}_attention_mask"],
                        "labels": feature[f"{key}_labels"],
                        "images": neg_images_per_sample[sample_idx],
                        "videos": feature["videos"],
                        "audios": feature["audios"],
                    }
                )

        neg_batch = MultiModalDataCollatorForSeq2Seq.__call__(self, neg_features)

        for key in _MM_IMAGE_DERIVED_KEYS:
            if key in neg_batch:
                pos_batch[f"neg_{key}"] = neg_batch[key]

        if has_sample_id:
            pos_batch["sample_id"] = torch.tensor(sample_ids, dtype=torch.long)
